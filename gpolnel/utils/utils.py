...
# +++++++++++++++++ Data Load
def load_cleaned_data(X_y=False, device='cpu'):
    """ Loads and returns the given data set for the project

    Basic information:
    - Number of data instances: 177;
    - Number of input features: 14;
    - Target's range: [4.7, 5.06].
...
# +++++++++++++++++ Data Load with transformed outliers
def load_cleaned_transformed_data(X_y=False, device='cpu'):
    """ Loads and returns the given data set for the project
    ...
    pandas.DataFrame
        ...
        target is the last column.
    """
    df = pd.read_csv(os.path.join(os.path.dirname(os.path.realpath(__file__)), "data","dt_cleaned_filtered_outliers.csv"))
    if X_y:
        return torch.from_numpy(df.values[:, :-1]).float().to(device), torch.from_numpy(df.values[:, -1]).float().to(device)
    else:
        return df
