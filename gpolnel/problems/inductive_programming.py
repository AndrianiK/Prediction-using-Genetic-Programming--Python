...
# Experimental approach to prediction with GSGP
    def predict_sol_gsgp(self, repr_, ffunction, test_idx, device, min_=None):
       
       """Predicts the output of the solution with representation repr_
        using the dataset given by test_idx.
        ...
        test_idx : torch.Tensor
            The indices of the test data partition.
        device : string
            The device in which the solution should be evaluated.
        min_ : bool
            A flag which defines the purpose of optimization.
       ...
       valid = True
        # Computes solution fitnesses
       if ffunction.is_structural:
                fit = ffunction(sol=repr_).clone().detach().to(device)
       else:
                if test_idx is None:
                 return None
                else:
                 y_pred = _execute_tree(repr_, self.X[test_idx])
                 fit = ffunction(call='semantic', y_true=self.y[test_idx], y_pred=y_pred)
       return fit
